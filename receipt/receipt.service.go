...
func handleDownload(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Disposition", "attachmen; filename="+ fileName)
...
func SetupRoute(apiBasePath string){
	handleReceipts := http.HandlerFunc(handleReceipts)
	handleDownload := http.HandlerFunc(handleDownload)
	http.Handle(fmt.Sprintf("%s/%s/", apiBasePath, receiptPath), cors.Middleware(handleReceipts))
	http.Handle(fmt.Sprintf("%s/%s/", apiBasePath, receiptPath), cors.Middleware(handleDownload))
}